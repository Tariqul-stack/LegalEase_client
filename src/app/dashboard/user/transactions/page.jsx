...
function UserTransactionsContent() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      const hirings = await response.json();
      const paidHirings = hirings.filter((h) => h.isPaid === true);

      setTransactions(paidHirings);
    } catch {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-800">My Transactions</h1>
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead>
              <tr className="bg-gray-50">
                {['Lawyer Name', 'Specialization', 'Amount', 'Date', 'Status'].map((heading) => (
                  <th
                    key={heading}
